...
-- type @a@ on success.
--
-- Normally, a permutation parser is first built with special
-- operators like ('<||>') and than transformed into a normal parser
-- using 'permute'.
...
permute (Perm def xs) = choice (map branch xs ++ none)
  where none = case def of Nothing -> []
                           Just x  -> [pure x]
